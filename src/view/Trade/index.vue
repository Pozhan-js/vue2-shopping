<template>
  <div class="trade-container">
    ...
    <div class="sub clearFix">
      <a class="subBtn" @click="getTradeOrder">提交订单</a>
    </div>
  </div>
</template>
<script>
...
export default {
  ...
  methods: {
    ...mapActions("trader", ["getTradeInfoData"]),
    async getTradeOrder() {
      const params = {
        consignee: "张三",
        consigneeTel: "13212345678",
        deliveryAddress: "深圳市",
        paymentWay: "ONLINE",
      ...
      const subBtnData = await reqSubmitOrder(this.tradeNo, params);
      // console.log("提交请求", subBtnData, this.tradeNo);
      if (subBtnData.code === 200) {
        // 提示用户提交成功
        this.$router.push({
          name: "pay",
          query: {
            orderId: subBtnData.data,
            money: this.totalAmount - 66,
          },
        });
      } else {
        // console.log("提交订单失败...");
  ...
  async mounted() {
    ...
    await this.getTradeInfoData();
    // this.$store.dispatch("trader/getUserAddressList");
    // this.$store.dispatch("trader/getTradeInfoData");
    // const data = await reqTradeInfoData();
    // console.log(data);
  },
};
</script>
