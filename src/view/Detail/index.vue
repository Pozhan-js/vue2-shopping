          ...
          <div class="choose">
            ...
            <div class="cartWrap">
              <div class="controls">
                ...
                <a
                  href="javascript:;"
                  :class="{ plus: true, disabled: count === 200 }"
                  @click="count++"
                  >+</a
                >
                <a
                  href="javascript:;"
                  :class="{ mins: true, disabled: count === 1 }"
                  @click="count--"
                  >-</a
                >
              </div>
...
import { skuNumReg } from "@/utils/reg.js";
...
export default {
  ...
  props: ["id"], //因为在routes文件中添加了配置 props：true的属性 路由规则中的params传参会直接被props接收
  components: {
    ImageList,
    Zoom,
  },
  ...
  methods: {
    async addSku2Cart() {
      ...
      if (result.code === 200) {
        ...
        // 因为在添加购物车成功页面有需要skuInfo内容展示 为了实现跨页面数据共享
    ...
    handlerActive(current, parentAttr) {
      //将同级元素全部isChecked属性全部设置为 0
      parentAttr.forEach((value) => {
        value.isChecked = "0";
      });
      current.isChecked = "1";
    },
